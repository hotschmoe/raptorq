...
test "PI solver with K'=18 constraint matrix" {
    ...
    var matrices = try cm.buildConstraintMatrices(allocator, kp);
    ...
    try pi_solver.solve(allocator, &matrices, &d, kp);
...
test "PI solver underdetermined detection" {
    ...
    const result = pi_solver.solve(allocator, &matrices, &buf, kp);
